(() => {
  ...
  function stop(reason="pause"){
    if(state.transition?.startedAt!==null){
      state.transition.elapsed=Math.min(
        ...
        performance.now()-state.transition.startedAt,
      );
  ...
  function render(turn){
    ...
    prev.disabled=state.turn===0;
    reset.disabled=state.turn===0;
    next.disabled=state.turn>=state.result.turn_count;
  ...
  function completeTransition(){
    ...
    window.uiLog?.("animation:complete",{
      from_turn:state.transition.fromTurn,
      to_turn:target,
    });
    state.transition=null;
    state.turn=target;
    ...
    reset.disabled=target===0;
    next.disabled=target>=state.result.turn_count;
  ...
  function prepareTransition(){
    ...
    window.uiLog?.("animation:start",{
      from_turn:state.turn,
      to_turn:state.turn+1,
      duration_ms:state.transition.duration,
    });
    return true;
  }
  ...
  function frame(timestamp){
    ...
    if(transition.startedAt===null){
      transition.startedAt=timestamp-transition.elapsed;
    }
    ...
    if(progress>=1){
      completeTransition();
      if(state.turn>=state.result.turn_count){
        state.playing=false;
        updatePlayButton();
        window.uiLog?.("playback:complete",{turn:state.turn});
        return;
      }
  ...
  function advanceReducedMotion(){
    ...
    if(state.playing){
      state.frame=requestAnimationFrame(()=>{
        window.setTimeout(advanceReducedMotion,duration());
      });
    }
  }
  ...
  function playAnimation(){
    ...
    state.playing=true;
    updatePlayButton();
    window.uiLog?.("playback:play",{turn:state.turn,duration_ms:duration()});
    if(reducedMotion.matches){
      window.uiLog?.("animation:reduced-motion",{enabled:true});
      window.setTimeout(advanceReducedMotion,duration());
      return;
    }
  ...
  function toggle(){
    if(state.playing){
      stop("pause");
      window.uiLog?.("playback:pause",{turn:state.turn});
      return;
    }
    playAnimation();
  }
  ...
  function load(result,map){
    ...
    state.result=result;
    state.turn=0;
    q("#workspace-map-name").textContent=`#${map.index} · ${map.display_path}`;
    slider.min="0";
    slider.max=String(result.turn_count);
    slider.value="0";
    slider.disabled=false;
    speed.disabled=false;
    play.disabled=result.turn_count===0;
    next.disabled=result.turn_count===0;
    window.renderFlyInOutput(result);
    workspace.classList.remove("is-hidden");
    render(0);
    workspace.scrollIntoView({behavior:"smooth",block:"start"});
    requestAnimationFrame(()=>window.FlyInCanvas?.fit());
  }
  ...
  function clear(){
    ...
    window.uiLog?.("simulation:cleared");
  }

  reset.addEventListener("click",()=>{
    stop("reset");render(0);window.uiLog?.("playback:reset");
  });
  prev.addEventListener("click",()=>{
    stop("previous");render(state.turn-1);window.uiLog?.("playback:previous",{turn:state.turn});
  });
  next.addEventListener("click",()=>{
    stop("next");render(state.turn+1);window.uiLog?.("playback:next",{turn:state.turn});
  });
  play.addEventListener("click",toggle);
  slider.addEventListener("input",()=>{
    stop("seek");render(Number(slider.value));window.uiLog?.("playback:seek",{turn:state.turn});
  });
  speed.addEventListener("change",()=>{
    if(state.transition){
      const fraction=state.transition.duration===0?0:state.transition.elapsed/state.transition.duration;
      state.transition.duration=duration();
      state.transition.elapsed=fraction*state.transition.duration;
      state.transition.startedAt=null;
    }
...
(() => {
  ...
  plus.addEventListener("click",()=>zoom(scale+step));
  minus.addEventListener("click",()=>zoom(scale-step));
