(() => {
  ...
  function load(result,map){stop();s.result=result;s.turn=0;q("#workspace-map-name").textContent=`#${map.index} · ${map.display_path}`;slider.min="0";slider.max=String(result.turn_count);slider.value="0";slider.disabled=false;speed.disabled=false;play.disabled=result.turn_count===0;next.disabled=result.turn_count===0;window.renderFlyInOutput(result);workspace.classList.remove("is-hidden");render(0);workspace.scrollIntoView({behavior:"smooth",block:"start"});requestAnimationFrame(()=>window.FlyInCanvas?.reset())}
...
(() => {
  ...
  function reset(){scale=1;apply();viewport.scrollLeft=0;viewport.scrollTop=Math.max(0,(surface.offsetHeight-viewport.clientHeight)/2);window.uiLog?.("canvas:reset",{scale:1,left:viewport.scrollLeft,top:viewport.scrollTop})}
  ...
  window.FlyInCanvas={fit,reset,setBaseSize};apply();
})();