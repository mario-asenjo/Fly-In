...
| --- | --- | --- | --- |
| README | First line, description, instructions, resources/AI use, algorithm, visual docs, example | Root README documents current M6 CLI/application state, commands, examples, architecture, benchmarks, AI use | EVIDENCED |
| OOP | Proper separation/encapsulation, justified inheritance only | Application/CLI adapter boundary plus package design explanation | EVIDENCED |
| Type safety | `mypy .` no errors/full hints | `make lint` and `make lint-strict` evidence through M6 | EVIDENCED |
...
| Visualization | Clear positions/movement/colors/capacity | `--visual` covers positions/movement/colors/static capacity/metrics; `--capacity-info` covers per-turn zone/link use | EVIDENCED |
| Basic scenarios | Single/multiple/maps/output/stationary omission | M3.6 known-route exact output tests plus M6 CLI/application tests | EVIDENCED |
...
| Explanation | Complexity, design, capacity/optimization trade-offs | M5 closure report, teaching notes, README architecture/benchmark sections | EVIDENCED |
...
| Edge cases | Single, bottleneck, disconnected, invalid links, zero/high capacity | Parser edge cases, M2.6 blocked/loop/dead-end path tests, M4/M5 capacity/deadlock tests | EVIDENCED |
| Error handling | Clear invalid/disconnected errors | Parser errors, M2.6 `NoRouteError`, and M6 CLI stderr/exit-code tests | EVIDENCED |
| Code quality | Structure, comments/docs/style, visual integration | Raw default `flake8 .`, mypy strict, context validation, Ponytail | EVIDENCED |
| Live coding | Add `--capacity-info` output within 10 minutes | Implemented as explicit CLI seam over `SolveResult.capacity_turns`; rehearsal command documented | EVIDENCED |
...
- Simulation result/turn snapshot exposes occupancy without reparsing text through
  `SolveResult.capacity_turns`.
- `format_capacity_info()` adds the diagnostic view.
- Default output remains unchanged.
- Rehearse from a clean branch/clone and time it during defense preparation.

Do not commit a fake “live modification” history. The goal is understanding and a clean seam.

## Benchmark authority

Use Fly-In 1.5 and the evaluation sheet values with the confirmed official package under the
category folders in `maps/`. Its hashes are pinned in `docs/sources/MANIFEST.sha256`; historical
v1.2 count conflicts remain documented without keeping a duplicate evaluator-facing map tree.
